...
mod tests {
    ...
    fn test_block() -> anyhow::Result<()> {
        ...
        let body = &et.protocol_state.t.t.body.t.t;
        assert_eq!(
        ...
        assert_eq!(*consensus_state.min_window_density, 33);
        assert_eq!(
        ...
        assert_eq!(*consensus_state.curr_global_slot.slots_per_epoch, 7140);
        assert_eq!(*consensus_state.global_slot_since_genesis, 111965);
        ...
        let constants = &body.constants.t.t;
        assert_eq!(constants.k.0, 290);
        assert_eq!(constants.slots_per_epoch.0, 7140);
        assert_eq!(constants.slots_per_sub_window.0, 7);
        assert_eq!(constants.delta.0, 0);
        assert_eq!(
            constants.genesis_state_timestamp.epoch_millis(),
            1615939200000
        );
        ...
        assert!(&et.staged_ledger_diff.t.diff.t.1.is_none());
        let commands = &et.staged_ledger_diff.t.diff.t.0.t.t.commands;
        ...
        match &commands[0].t.data.t.t {
            mina_rs_base::network_types::staged_ledger_diff::UserCommand::SignedCommand(
                ...
                let bytes = bs58::decode("B62qoSuxNqwogusxxZbs3gpJUxCCN4GZEv21FX8S2DtNpToLgKnrexM")
                    .into_vec()
                    .unwrap();
                assert_eq!(command.t.t.signer.x[..], bytes[3..35]);

                assert_eq!(command.t.t.signature.to_base58_string(), "7mXTB1bcHYLJTmTfMtTboo4FSGStvera3z2wd6qjSxhpz1hZFMZZjcyaWAFEmZhgbq6DqVqGsNodnYKsCbMAq7D8yWo5bRSd");
                let bytes = bs58::decode("7mXTB1bcHYLJTmTfMtTboo4FSGStvera3z2wd6qjSxhpz1hZFMZZjcyaWAFEmZhgbq6DqVqGsNodnYKsCbMAq7D8yWo5bRSd")
                    .into_vec()
                    .unwrap();
                assert_eq!(command.t.t.signature.field_point().as_ref(), &bytes[2..34]);
                assert_eq!(
                    command.t.t.signature.inner_curve_scalar().as_ref(),
                    &bytes[34..66]
                );

                assert_eq!(command.t.t.payload.t.t.common.t.t.t.nonce.0, 5694);
                assert_eq!(
                ...
                match &command.t.t.payload.t.t.body.t.t {
                    mina_rs_base::network_types::staged_ledger_diff::SignedCommandPayloadBody::PaymentPayload(body) => {
                        assert_eq!(body.t.t.amount.to_string(), "0.027370000");
                        let bytes =
                            bs58::decode("B62qoSuxNqwogusxxZbs3gpJUxCCN4GZEv21FX8S2DtNpToLgKnrexM")
                                .into_vec()
                                .unwrap();
                        // TODO: Validate full bytes vec with salted mainnet signature
                        assert_eq!(body.t.t.source_pk.x[..], bytes[3..35]);
                        let bytes =
                            bs58::decode("B62qn2MtuQ9GyyVnotUHB9Ehp9EZre5m6TYpGx64tBCDHHBZFZRURnL")
                                .into_vec()
                                .unwrap();
                        // TODO: Validate full bytes vec with salted mainnet signature
                        assert_eq!(body.t.t.receiver_pk.x[..], bytes[3..35]);
                        assert_eq!(body.t.t.token_id.0, 1);
                    }
                    _ => bail!(
                        "PaymentPayload expected, but found: {:#?}",
                        command.t.t.payload.t.t.body
                    ),
                };
            }
            _ => bail!(
        ...
        let coinbase = &et.staged_ledger_diff.t.diff.t.0.t.t.coinbase;
        match coinbase.t {
            mina_rs_base::network_types::staged_ledger_diff::CoinBase::One(_) => {}
