...
mod tests {
    ...
    fn test_block() -> anyhow::Result<()> {
        ...
        assert_eq!(
            SnarkedLedgerHash::from(blockchain_state.t.t.snarked_ledger_hash.clone()).to_base58_string(),
            "jxkQm8ge9sYPwPyUYUMZ6wr7SQ6Pit5szbRvPmEzYKQQZAnACyC"
        );
        assert_eq!(
            SnarkedLedgerHash::from(blockchain_state.t.t.genesis_ledger_hash.clone()).to_base58_string(),
            "jx7buQVWFLsXTtzRgSxbYcT8EYLS8KCZbLrfDcJxMtyy4thw2Ee"
        );
        assert_eq!(blockchain_state.t.t.snarked_next_available_token.t.t.t, 2);
        assert_eq!(BlockTime::from(blockchain_state.t.t.timestamp.clone()).epoch_millis(), 1636092900000);
        assert_eq!(
        ...
        .unwrap();
        assert_eq!(VrfOutputTruncated::from(consensus_state.t.t.last_vrf_output.t.clone()).as_ref(), &bytes[..]);

        assert_eq!(
            Amount::from(consensus_state.t.t.total_currency.t.t).to_string(),
            "867667132.840039233"
        );
        assert_eq!(consensus_state.t.t.curr_global_slot.t.t.slot_number.t.t, 111965);
        assert_eq!(consensus_state.t.t.curr_global_slot.t.t.slots_per_epoch.t.t, 7140);
        ...
        */
        ...
        assert!(&StagedLedgerDiffTuple::from(et.0.t.staged_ledger_diff.t.diff.clone()).diff_one().is_none());
        let commands = StagedLedgerDiffTuple::from(et.0.t.staged_ledger_diff.t.diff.clone()).diff_two().commands.clone();
        ...
        let coinbase = StagedLedgerDiffTuple::from(et.0.t.staged_ledger_diff.t.diff.clone()).diff_two().coinbase.clone();
        match coinbase {
            CoinBase::One(_) => {}
            _ => bail!("CoinBase::One expected, but found: {:#?}", coinbase),
        };

        let internal_commands = StagedLedgerDiffTuple::from(et
            .0.t.staged_ledger_diff
            .t.diff.clone())
            .diff_two()
            .internal_command_balances.clone();
