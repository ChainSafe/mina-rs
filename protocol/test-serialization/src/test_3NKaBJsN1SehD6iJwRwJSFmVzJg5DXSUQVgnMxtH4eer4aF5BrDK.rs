...
mod tests {
    ...
    fn test_block() -> anyhow::Result<()> {
        ...
        assert_eq!(
            protocol_state.t.t.previous_state_hash.to_base58_string(),
            "3NKDdX6eVtAgmmTVxaFLnnPPrsGKgVepG2k5cf8HocgSw6ps8Sww"
        );

        let body = &et.0.t.protocol_state.t.t.body;
        assert_eq!(
            body.t.t.genesis_state_hash.to_base58_string(),
        ...
        assert_eq!(
            non_snark.t.ledger_hash.to_base58_string(),
        ...
        assert_eq!(
            non_snark.t.aux_hash.to_base58_string(),
        ...
        assert_eq!(
            non_snark.t.pending_coinbase_aux.to_base58_string(),
        ...
        assert_eq!(
            blockchain_state
                ...
                .t.t.t.pending_coinbase_hash
                .to_base58_string(),
            "2mzpdUi5ddLicLGUns4iYFiNahL5B5cPkTUot83v2moNtr4mzRYf"
        );
        assert_eq!(
            blockchain_state.t.t.snarked_ledger_hash.to_base58_string(),
            "jxkQm8ge9sYPwPyUYUMZ6wr7SQ6Pit5szbRvPmEzYKQQZAnACyC"
        );
        assert_eq!(
            blockchain_state.t.t.genesis_ledger_hash.to_base58_string(),
            "jx7buQVWFLsXTtzRgSxbYcT8EYLS8KCZbLrfDcJxMtyy4thw2Ee"
        );
        assert_eq!(blockchain_state.t.t.snarked_next_available_token.t, 2);
        assert_eq!(blockchain_state.t.t.timestamp.epoch_millis(), 1636092900000);
        assert_eq!(
            blockchain_state.t.t.timestamp.datetime(),
            datetime!(2021-11-05 06:15:00 UTC)
        );

        let consensus_state = &body.t.t.consensus_state;
        assert_eq!(*consensus_state.t.t.blockchain_length, 77748);
        assert_eq!(*consensus_state.t.t.epoch_count, 15);
        assert_eq!(*consensus_state.t.t.min_window_density, 33);
        assert_eq!(
            consensus_state.sub_window_densities(),
        ...
        .unwrap();
        assert_eq!(consensus_state.t.t.last_vrf_output.as_ref(), &bytes[..]);

        assert_eq!(
            consensus_state.t.t.total_currency.to_string(),
            "867667132.840039233"
        );
        assert_eq!(*consensus_state.t.t.curr_global_slot.t.t.slot_number, 111965);
        assert_eq!(*consensus_state.t.t.curr_global_slot.t.t.slots_per_epoch, 7140);
        assert_eq!(*consensus_state.t.t.global_slot_since_genesis, 111965);

        let staking_epoch_data = &consensus_state.t.t.staking_epoch_data;
        assert_eq!(
            staking_epoch_data.t.t.ledger.t.t.hash.to_base58_string(),
            "jxn15ATGoe4WGgYpbssxJH9XW8NXRDy22WvSsBqvMqcnLPgPAwN"
        );
        assert_eq!(
            staking_epoch_data.t.t.ledger.t.t.total_currency.to_string(),
            "861208012.840039233"
        );
        assert_eq!(
            staking_epoch_data.t.t.seed.to_base58_string(),
            "2vao4i3odTHZVRbEhdkKvLoD1rW2UuiVaayVFosYtkghABg29o7i"
        );
        assert_eq!(
            staking_epoch_data.t.t.start_checkpoint.to_base58_string(),
            "3NLM6x7j2Z68e8gGspyvc1aU884uU6yWkwz9aW127BFckn9b5uvo"
        );
        assert_eq!(
            staking_epoch_data.t.t.lock_checkpoint.to_base58_string(),
            "3NLiFhztdCsuWSociNGMspidiYkyqNKZw6ufH7jqbgQtEgGtBb2P"
        );
        assert_eq!(*staking_epoch_data.t.t.epoch_length, 4697);

        let next_epoch_data = &consensus_state.t.t.next_epoch_data;
        assert_eq!(
            next_epoch_data.t.t.ledger.hash.to_base58_string(),
            "jwAXd4GZgxE3YCwqs99g4MpLNiEV2ZfZPstyah4jxo753AVgL6R"
        );
        assert_eq!(
            next_epoch_data.t.t.ledger.total_currency.to_string(),
            "864998092.840039233"
        );
        assert_eq!(
            next_epoch_data.t.t.seed.to_base58_string(),
            "2vbUkQGF5swXK7PNaAJDUQirW1fbZiUJDzbBKwfPGdJXZiryburD"
        );
        assert_eq!(
            next_epoch_data.t.t.start_checkpoint.to_base58_string(),
            "3NLkdXKqoHfwZ5jT1uxSY3eoFy3C2jpAUFZ1Y6eSMsE66MNJqErx"
        );
        assert_eq!(
            next_epoch_data.t.t.lock_checkpoint.to_base58_string(),
            "3NLW5kBi9nXDzzdr2C3p9X6QaKaASMaVHp3otwreKXKJToUNK7yu"
        );
        assert_eq!(*next_epoch_data.epoch_length, 3285);

        assert_eq!(consensus_state.t.t.has_ancestor_in_same_checkpoint_window, true);
        assert_eq!(
            consensus_state.t.t.block_stake_winner.to_base58_string(),
            "B62qmsYXFNNE565yv7bEMPsPnpRCsMErf7J2v5jMnuKQ1jgwZS8BzXS"
        );
        assert_eq!(
            consensus_state.t.t.block_creator.to_base58_string(),
            "B62qpge4uMq4Vv5Rvc8Gw9qSquUYd6xoW1pz7HQkMSHm6h1o7pvLPAN"
        );
        assert_eq!(
            consensus_state.t.t.coinbase_receiver.to_base58_string(),
        ...
        assert_eq!(constants.t.t.slots_per_epoch.0, 7140);
        assert_eq!(constants.t.t.slots_per_sub_window.0, 7);
        assert_eq!(constants.t.t.delta.0, 0);
        assert_eq!(
            constants.t.t.genesis_state_timestamp.epoch_millis(),
            1615939200000
        );
        ...
        assert!(&et.0.t.staged_ledger_diff.t.diff.diff_one().is_none());
        let commands = &et.0.t.staged_ledger_diff.t.diff.diff_two().commands;
        ...
        let coinbase = &et.0.t.staged_ledger_diff.t.diff.diff_two().coinbase;
        match coinbase {
            CoinBase::One(_) => {}
        ...
        let bytes = bs58::decode("jwHLk8kaC6B45K3sjuX2sM38649VtfpUAteTfKFQMPcqTeXjGiT")
            .into_vec()
            .unwrap();
        assert_eq!(et.0.t.delta_transition_chain_proof.0.t.as_ref()[..], bytes[2..34]);
        ...
        assert_eq!(et.0.t.current_protocol_version, ProtocolVersion::default().into());
