...
#[wasm_bindgen(js_name = runProcessor)]
pub async fn run_processor() {
    frontier::PROCESSOR_BERKELEY.run().await;
}

#[wasm_bindgen(js_name = setApiList)]
pub async fn set_api_list(api_urls: Array) -> Result<bool, JsError> {
...
#[wasm_bindgen(js_name = setTrackingAccounts)]
pub async fn set_tracking_accounts(accounts: Array) -> Result<bool, JsError> {
    let mut backend = frontier::PROCESSOR_BERKELEY.nonconsensus_ops_mut().await;
...
#[wasm_bindgen(js_name = pollLatestBlocks)]
...
#[wasm_bindgen(js_name = getBestChainStateHash)]
...
#[wasm_bindgen(js_name = getBestChainState)]
...
#[wasm_bindgen(js_name = getSparseMerkleLedger)]
